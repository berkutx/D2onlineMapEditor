 ...
 * that actually have parameters — terrain (land type + brush size) and
 * water/forest/erase (brush size). For every other tool it isn't rendered at
 * all, so the canvas keeps its full width and there's no empty panel. It floats
 * (absolute), so appearing/disappearing never reflows the canvas.
 */
...
const visible = computed(() => SIZED.has(tool.value));
</script>
